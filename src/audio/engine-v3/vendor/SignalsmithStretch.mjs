...
function registerWorkletProcessor(Module, audioNodeKey) {
	class WasmProcessor extends AudioWorkletProcessor {
		constructor(options) {
			...
			this._latched = false
			// 067-E: instrumentation for REGIME 0 harness — NO DSP changes
			...
			this._instr = {
				...
				maxBufferLength: 0,
				minInputSamplesEnd: Infinity,
				maxInputSamplesEnd: 0,
				audioBuffersStart: 0,
				audioBuffersEnd: 0,
				audioBuffersCount: 0,
				audioBufferIndex_max: 0,
				lastRate: 1,
				times: [],
				_warnedInput: false,
				// DR-077 Gate 2: dropout detection
				dropoutCount: 0,
				_lastProcessTime: 0,
				_expectedInterval: 0,
				// 067-E v6.7 (Gate 3 verdict): copy/skip/blockSamples/RMS — measurement only
				copyCount: 0,
				skipCount: 0,
				maxBlockSamples: 0,
				lastBlockSamples: 0,
				...
				maxBuffersOutRms: 0,
				lastBuffersOutRms: 0,
				// v6.19 (Opus Variant B): vendorStartFrame + диагностика Дефекта B — measurement only
				vendorStartFrame: -1,     // currentFrame в момент фактического принятия start
				startCount: 0,            // сколько раз принят start
				configureCount: 0,        // сколько раз выполнен configure()
				resetCount: 0,            // сколько раз вызван wasmModule._reset()
				seekCount: 0,             // сколько раз вызван _seek
				lastSeekFrame: -1,        // кадр последнего _seek
				firstInputBlockFrame: -1, // кадр первого process с blockSamples > 0 (данные дошли до WASM)
				firstOutputBlockFrame: -1,// кадр первого process с outRms > 0 (WASM выдал звук)
				quantumTrace: [],         // [{f, in, out}] кванты с данными (лимит 1000)
				// v6.20 (MICRO-PACK-080): трассировка Дефекта B — measurement only
				seekTrace: [],            // вызовы _seek/_process (лимит 200)
				abortSnapshot: null,      // single-shot снимок обрыва
				_abortSnapshotTaken: false, // флаг single-shot снимка
			}
			
			let remoteMethods = {
				// 067-E: instrumentation — measurement only, NO DSP change
				setInstrumentation: on => { this._instrument = on },
				getInstrumentMetrics: _ => {
					const m = this._instr
					return {
						processCalls: m.processCalls,
						blockSize: { min: m.minBlockSize, max: m.maxBlockSize },
						bufferLength: { min: m.minBufferLength, max: m.maxBufferLength },
						inputSamplesEnd: { min: m.minInputSamplesEnd, max: m.maxInputSamplesEnd },
						audioBuffersStart: m.audioBuffersStart,
						audioBuffersEnd: m.audioBuffersEnd,
						audioBuffersCount: m.audioBuffersCount,
					audioBufferIndex_max: m.audioBufferIndex_max,
					lastRate: m.lastRate,
					sampleRate: sampleRate,
					latencySeconds: this.inputLatencySeconds + this.outputLatencySeconds,
					latencySamples: Math.round((this.inputLatencySeconds + this.outputLatencySeconds) * sampleRate),
					inputLatencySeconds: this.inputLatencySeconds,
					outputLatencySeconds: this.outputLatencySeconds,
					// DR-077 Gate 2: dropout
					dropoutCount: m.dropoutCount,
					// 067-E v6.7 (Gate 3 verdict): copy/skip/blockSamples/RMS — measurement only
					copyCount: m.copyCount,
					skipCount: m.skipCount,
					blockSamples: { last: m.lastBlockSamples, max: m.maxBlockSamples },
					buffersInRms: { last: m.lastBuffersInRms, max: m.maxBuffersInRms },
					buffersOutRms: { last: m.lastBuffersOutRms, max: m.maxBuffersOutRms },
					// v6.19 (Opus Variant B): диагностика
					vendorStartFrame: m.vendorStartFrame,
					startCount: m.startCount,
					configureCount: m.configureCount,
					resetCount: m.resetCount,
					seekCount: m.seekCount,
					lastSeekFrame: m.lastSeekFrame,
					firstInputBlockFrame: m.firstInputBlockFrame,
					firstOutputBlockFrame: m.firstOutputBlockFrame,
					quantumTrace: m.quantumTrace,
					// v6.20 (MICRO-PACK-080): трассировка Дефекта B — measurement only
					seekTrace: m.seekTrace ?? [],
					abortSnapshot: m.abortSnapshot ?? null,
				}
				},
				configure: config => {
				...
				start: (when, offset, duration, rate, semitones) => {
					// v6.19 (Opus Variant B): фиксируем момент фактического принятия start в vendor
					// через глобальный currentFrame (единый audio clock AudioContext) — measurement only
					if (this._instrument) {
						this._instr.startCount++
						this._instr.vendorStartFrame = currentFrame
					}
				...
				schedule: (objIn, adjustPrevious) => {
					// 067-B: компенсируем outputLatency для объектного пути.
					// Приоритет: outputTime (явный ключ) → output (используется start()/stop()) → currentTime + outputLatency
					// Без этого каждый seek/start/scheduleRate уезжает на outLat × rate,
					// а stop() теряет позицию остановки.
					let outputTime = objIn.outputTime ?? objIn.output ?? currentTime + this.outputLatencySeconds;
		...
		configure() {
			this._instr.configureCount++   // v6.19: diagnostic (measurement only)
			if (this.config.blockMs) {
				...
				this._instr.resetCount++   // v6.19: diagnostic (measurement only)
		...
		process(inputList, outputList, parameters) {
			try {
			const instr = this._instrument
			// v6.20 (MICRO-PACK-080): локальные дельты copy/skip за квант — measurement only
			let copyDelta = 0, skipDelta = 0;
			// 067-E: instrumentation — collect metrics, NO DSP change
			if (instr) {
					this._instr.processCalls++
					// DR-077 Gate 2: dropout detection
					if (this._instr._lastProcessTime > 0) {
						const blockSamples = outputList[0][0].length || 128
						const expectedInterval = blockSamples / sampleRate
						const gap = currentTime - this._instr._lastProcessTime
						if (expectedInterval > 0 && gap > expectedInterval * 1.5) {
							this._instr.dropoutCount++
						}
					}
					this._instr._lastProcessTime = currentTime
					this._instr._expectedInterval = (outputList[0][0].length || 128) / sampleRate
				}

				// Если залатчены — тишина
				if (this._latched) {
				...
				let currentMapSegment = this.timeMap[0];
				// 067-E: instrument rate
				if (instr) {
					this._instr.lastRate = currentMapSegment.rate
					this._instr.audioBuffersStart = this.audioBuffersStart
					this._instr.audioBuffersEnd = this.audioBuffersEnd
					this._instr.audioBuffersCount = this.audioBuffers.length
				}
				...
				if (!currentMapSegment.active) {
					...
					// 067-E: instrument samples/block metrics — NO DSP change
					if (instr) {
						const bs = outputList[0][0].length
						if (bs < this._instr.minBlockSize) this._instr.minBlockSize = bs
						if (bs > this._instr.maxBlockSize) this._instr.maxBlockSize = bs
						if (inputSamplesEnd < this._instr.minInputSamplesEnd) this._instr.minInputSamplesEnd = inputSamplesEnd
						if (inputSamplesEnd > this._instr.maxInputSamplesEnd) this._instr.maxInputSamplesEnd = inputSamplesEnd
						if (this.bufferLength < this._instr.minBufferLength) this._instr.minBufferLength = this.bufferLength
						if (this.bufferLength > this._instr.maxBufferLength) this._instr.maxBufferLength = this.bufferLength
					}
					...
					// 067-E: track max audioBufferIndex traversed
				if (instr && audioBufferIndex > this._instr.audioBufferIndex_max) {
					this._instr.audioBufferIndex_max = audioBufferIndex
				}
				while (audioBufferIndex < this.audioBuffers.length && audioSamples < inputSamplesEnd) {
						...
						if (count > 0) {
							// 067-E v6.7: count actual copies — measurement only
							if (instr) this._instr.copyCount++
							copyDelta++   // v6.20 (MICRO-PACK-080): дельта копий за квант — measurement only
							...
							// 067-E v6.7: count skipped buffers — measurement only
							if (instr) this._instr.skipCount++
							skipDelta++   // v6.20 (MICRO-PACK-080): дельта skip за квант — measurement only
					...
					// 067-E v6.7: blockSamples + RMS of buffersIn (what WASM will read) — measurement only
					if (instr) {
						this._instr.lastBlockSamples = blockSamples
						if (blockSamples > this._instr.maxBlockSamples) this._instr.maxBlockSamples = blockSamples
						if (blockSamples > 0) {
							let sumSq = 0, n = 0
							buffers.forEach(b => {
								for (let i = 0; i < blockSamples; i++) { sumSq += b[i] * b[i]; n++ }
							})
							const rms = Math.sqrt(sumSq / n)
							this._instr.lastBuffersInRms = rms
							if (rms > this._instr.maxBuffersInRms) this._instr.maxBuffersInRms = rms
						}
						if (blockSamples > 0 && this._instr.firstInputBlockFrame < 0) this._instr.firstInputBlockFrame = currentFrame
					}
					...
					if (instr) { this._instr.seekCount++; this._instr.lastSeekFrame = currentFrame }
					wasmModule._process(0, outputBlockSize);
					// v6.20 (MICRO-PACK-080): трассировка вызовов _seek/_process — measurement only
					if (instr && this._instr.seekTrace.length < 200) {
					  this._instr.seekTrace.push({
					    f: currentFrame,
					    seekPos: this.bufferLength,
					    rate: currentMapSegment.rate,
					    blockSamples: blockSamples,        // сколько данных было до seek
					    audioBuffersStart: this.audioBuffersStart,
					    audioBuffersEnd: this.audioBuffersEnd,
					    audioBuffersCount: this.audioBuffers.length,
					    audioBufferIndex_max: this._instr.audioBufferIndex_max,
				...
				// 067-E: instrument last blockSize and inputList presence — NO DSP change
				if (instr) {
					const hasInput = !!(inputList?.[0]?.[0]?.length)
					if (hasInput && !this._instr._warnedInput) {
						this._instr._warnedInput = true
						console.warn('[SS-INSTR] inputList[0] has data — unexpected for playback path')
					}
					// Track process calls count; currentTime from process() param, NOT performance.now() (not available in AudioWorklet)
					if (this._instr.times.length < 1000) {
						this._instr.times.push(currentTime)
					}
					// 067-E v6.7: RMS of buffersOut (what WASM produced) — measurement only
					let sumSq = 0, n = 0
					outputList[0].forEach(ch => {
						for (let i = 0; i < ch.length; i++) { sumSq += ch[i] * ch[i]; n++ }
					})
					const outRms = n > 0 ? Math.sqrt(sumSq / n) : 0
					this._instr.lastBuffersOutRms = outRms
					if (outRms > this._instr.maxBuffersOutRms) this._instr.maxBuffersOutRms = outRms
					if (outRms > 1e-6 && this._instr.firstOutputBlockFrame < 0) this._instr.firstOutputBlockFrame = currentFrame
					// v6.20 (MICRO-PACK-080): quantum trace — кванты с данными (для диагностики Дефекта B)
					if ((outRms > 1e-6 || this._instr.lastBuffersInRms > 1e-6) && this._instr.quantumTrace.length < 1000) {
						this._instr.quantumTrace.push({ f: currentFrame, in: this._instr.lastBuffersInRms, out: outRms, b: this._instr.lastBlockSamples, c: copyDelta, s: skipDelta, r: currentMapSegment.rate, e: this._errorCount, l: this._latched })
					}
					// v6.20 (MICRO-PACK-080): снимок обрыва Дефекта B — single-shot (первый квант с данными) — measurement only
					if (!this._instr._abortSnapshotTaken && this._instr.lastBlockSamples > 0 && outRms > 1e-6) {
					  this._instr._abortSnapshotTaken = true
					  this._instr.abortSnapshot = {
					    f: currentFrame,
					    copyCount: this._instr.copyCount, skipCount: this._instr.skipCount,
					    blockSamples: this._instr.lastBlockSamples, lastBlockSamples: this._instr.lastBlockSamples,
					    audioBuffersStart: this._instr.audioBuffersStart, audioBuffersEnd: this._instr.audioBuffersEnd, audioBuffersCount: this._instr.audioBuffersCount,
					    bufferLength: this.bufferLength,
					    inputLatencySeconds: this.inputLatencySeconds, outputLatencySeconds: this.outputLatencySeconds,
					    rate: currentMapSegment.rate,
					    inputSamplesEnd: this._instr.maxInputSamplesEnd,
					    errorCount: this._errorCount, latched: this._latched,
					    firstInputBlockFrame: this._instr.firstInputBlockFrame,
					    firstOutputBlockFrame: this._instr.firstOutputBlockFrame
					  }
					}
				}
				
				// A6: clean process → сбрасываем error counter
				this._errorCount = 0
				this._latched = false
				return true;
			} catch (e) {
				// A6: throttle — не чаще 1 ошибки в секунду
				const now = performance.now ? performance.now() : Date.now()
				this._errorCount++
				if (this._errorCount === 1) {
				  this._firstErrorTime = now
				}
				
				// Заполняем тишиной (как и раньше)
				if (outputList?.[0]) {
				  outputList.forEach(output => {
				    output.forEach(channel => {
				      try { channel.fill(0) } catch {}
				    })
				  })
				}
				
				// Если > 3 ошибок за 1 секунду — латчимся навсегда
				if (this._errorCount >= 3 && (now - this._firstErrorTime) < 1000) {
				  this._latched = true
				  console.warn('[SS] ⚠️ Too many errors — latching to silence')
				  // Не постим ошибку — молчим
				  return true
				}
				
				// normal error — не чаще 1/sec
				if (this._errorCount === 1 || (now - this._firstErrorTime) > 1000) {
				  this.port.postMessage(['error', {
				    msg: e?.message ?? String(e),
				    wasmReady: this.wasmReady,
				    channels: this.channels,
				    buffersLength: this.audioBuffers?.length,
				    inputListLength: inputList?.length,
				    timeMapLength: this.timeMap?.length
				  }])
				  if (this._errorCount > 1) {
				    this._firstErrorTime = now
				    this._errorCount = 1
				...
				return true // Живём — не возвращаем false
			}
		}
	}
