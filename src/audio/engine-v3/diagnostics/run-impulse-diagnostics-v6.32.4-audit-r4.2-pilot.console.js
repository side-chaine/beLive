;(async () => {
  ...
  const SCRIPT_VERSION = 'v6.32.4-audit-r4';
  const HARNESS_REVISION = 'v6.32.4-audit-r4';               // MAND 208: Condition B — STATE-PRESERVING REUSE lifecycle (deterministic drain), harness-only
  const INSTRUMENTATION_R4 = 'completion+state+coords+drain'; // v6.32.4-audit-r4: + per-row drain marker & lifecycle
  // MAND 208 §1: Condition B = POOL_STATE 'reused' (переиспользование vendor node+capture МЕЖДУ строками,
  // с детерминированным drain через stop→dropBuffers→cap.clear→(re)configure→start). Condition A
  // остаётся immutable baseline: fresh-per-row (r3). Обе ветки живут в этом harness.
  const CONFIG_CONDITION_ID = 'B';                        // Block B: default CONDITION B; fresh control series пометит строки A отдельно
  const POOL_STATE = 'reused';                            // Condition B: переиспользуемый пул node+capture между строками
  const HARNESS_SHA256 = null; // external manifest (П8 / 208)
  const SOURCE_PATH = 'src/audio/engine-v3/diagnostics/run-impulse-diagnostics-v6.32.4-audit-r4.console.js';
  ...
  const EXPERIMENT_ID = 'phase-plus-energy-profile-v6.32.4-audit-r4';   // MAND 208: Condition B reuse lifecycle
  ...
  const DELAY_POINTS_MS = [0, 2.5, 5, 7.5, 10];      // PILOT: 5 точек → 5 rows
  const REPS_PER_DELAY = 1;                          // PILOT: 1 rep per delay (5 rows total)
  ...
  window.__impulseResultV6324R4 = out;

  const log = (...x) => console.log('[v6.32.4-audit-r4]', ...x);
  ...
  async function runVendorRow(delayMs, delayIndex, rep) {
    ...
    try {
      ...
      const r3_startRowMs = tStartMs;               // completion: момент после start-комки
      ...
      const tRead = async (label) => {
        ...
        return { readPointId: label, elapsedMs, requestContextFrame: Math.round(ctx.currentTime * SR), requestedLength: CAPTURE_SIZE, returnedLength: (snap?.result?.length ?? null), windowMeta: meta };
      };
      const readPoints = [];
      ...
      // v6.32.3-audit-r3 (r3-instrumentation): completion/state/coords — observability-only, не меняет классификацию
      row.raw_observations.completion = {
        conditionId: CONFIG_CONDITION_ID,
        poolState: POOL_STATE,
        rowStartMs: r3_startRowMs,
        rowDoneMs: Math.round(performance.now()),
        rowDurationMs: Math.round(performance.now() - r3_startRowMs),
        deadlineHit,
        stopReason: row.verdict?.rowValidity === 'INVALID'
          ? ('invalid:' + String(row.verdict?.invalidReasons?.join('|') || 'unknown'))
          : (deadlineHit ? 'row_deadline' : 'completed'),
        vendorNodeConfiguredMs: BLOCK_MS,        // конфигурация узла (observability)
        origin: 'v6.32.3-audit-r3-instrumentation'
      };
      row.raw_observations.conditionId = CONFIG_CONDITION_ID;
      row.raw_observations.branchStateId = (row.verdict?.rowValidity === 'INVALID')
        ? ('invalid:' + String(row.verdict?.invalidReasons?.join('|') || 'unknown'))
        : (row.raw_observations?.rpc?.start?.__v627 ? 'valid-phase-path' : 'valid');
      row.raw_observations.captureRequestedLength = CAPTURE_SIZE;
      row.raw_observations.captureReturnedLength = Array.isArray(readRec?.result) ? readRec.result.length : (readRec?.result?.length ?? null);
      row.raw_observations.blockSamples = Math.round(BLOCK_MS * SR / 1000);
      row.raw_observations.intervalSamples = Math.round(INTERVAL_MS * SR / 1000);
      row.raw_observations.actualContextSampleRate = ctx.sampleRate;
      row.raw_observations.actualBufferSize = ctx.baseLatency != null ? Math.round(ctx.baseLatency) : null;
      return row;
    } finally {
  ...
  // ============================================================
  // MAND 208 §1 — CONDITION B (POOL_STATE 'reused'): deterministic
  // drain / state-preserving reuse lifecycle across a series of rows.
  //
  //   LIFECYCLE (per row in the series):
  //     1. PRIME  : ensure live pool { ctx, node, cap } — created ONCE.
  //     2. DRAIN  : stop() → dropBuffers() → cap.clear(+read==0) → section
  //                 drain marker (rows-to-next must be zero-residual).
  //     3. LOAD   : addBuffers(new click) for THIS row.
  //     4. GAP    : CLEAR_TO_START_GAP_MS (readiness, NOT timing contract).
  //     5. CONFIG  : configure() boundary (blockMs/intervalMs idempotent).
  //     6. START   : start({active:true, input:0, rate:1}) + start bounds.
  //     7. READ    : readWindow + stats + instrument metrics (same as r3).
  //     8. MARK    : per-row drain marker = {drained:bool, residualNonZero,
  //                 residualPeak, drainElapsedMs, lifecycle:'B'}.
  //
  // Condition A remains the immutable baseline (fresh-per-row). Condition B
  // PROVES reuse WITHOUT production changes (harness-only). No vendor/frozen
  // edits. If reuse were impossible without production edits, r4 would raise
  // IMPLEMENTATION_BLOCKED — we do NOT fake Condition B with a single flag.
  // ============================================================
  const R4_CONDITION_B = true;

  // Pool cells: each channel uses node 'n' + capture 'cap' reused across rows.
  // keepAlive = {ctx, cap, node, latency, firstRowDone}
  async function ensurePoolB() {
    if (poolB) return poolB;
    const ctx = new AudioContext({ sampleRate: SR });
    if (ctx.state === 'suspended') await ctx.resume();
    const url = URL.createObjectURL(new Blob([captureCode], { type: 'text/javascript' }));
    try { await ctx.audioWorklet.addModule(url); } finally { URL.revokeObjectURL(url); }
    ...
    await sleep(150);
    await rpc(node, 'setInstrumentation', [true]);
    poolB = { ctx, node, cap, latency, gain };
    return poolB;
  }
  let poolB = null;

  // Deterministic drain before each new row:
  //   - vendor stop (schedule inactive) to halt the time-map
  //   - dropBuffers () — flush any queued PCM
  //   - capture clear → assert zero-residual read (drain marker)
  async function drainPoolB(pool) {
    const marker = {
      condition: 'B', poolState: 'reused',
      stop: null, dropBuffers: null, cleared: null, residualPeak: 0, residualEnergy: 0,
      drained: false, cleared: [],
      elapsedMs: null,
    };
    if (!pool) return { ...marker, drained: false, reason: 'no-pool' };
    const t0 = performance.now();
    const st = await rpc(pool.node, 'stop', [{ active: false }]);
    marker.stop = st?.result ?? null;
    let dbRec = null;
    try { dbRec = await rpc(pool.node, 'dropBuffers', [0]); } catch (e) { marker.dropBuffersErr = String(e?.message || e); }
    marker.dropBuffers = dbRec?.result ?? null;
    const clr = await rpc(pool.cap, 'clear'); marker.cleared = clr?.result ?? null;
    // zero-residual proof: read whole current buffer region, ensure zero
    const probeLen = CAPTURE_SIZE;
    const probe = await rpc(pool.cap, 'readWindow', [0, probeLen]);
    const arr = probe?.result;
    let peak = 0, energy = 0;
    if (Array.isArray(arr) || arr instanceof Float32Array) {
      for (let i = 0; i < arr.length; i++) { const a = Math.abs(arr[i]); if (a > peak) peak = a; energy += arr[i] * arr[i]; }
    }
    marker.residualPeak = peak; marker.residualEnergy = energy;
    marker.drained = peak < 1e-6 && energy < 1e-9;
    marker.elapsedMs = Math.round(performance.now() - t0);
    return marker;
  }

  // One Condition B vendor row: reuse pool, drain first, then stim/read.
  async function runVendorRowB(delayMs, delayIndex, rep) {
    const pool = await ensurePoolB();                 // reused across rows (created once)
    const offset = PHASE_OFFSET;
    const rpcRecs = {};
    const drainMarker = await drainPoolB(pool);      // deterministic drain (new audio starts clean)
    // config boundary (idempotent back-to-back reuse step)
    const cfgRec = await rpc(pool.node, 'configure', [{ blockMs: BLOCK_MS, intervalMs: INTERVAL_MS, splitComputation: SPLIT_COMPUTATION }]);
    rpcRecs.configureB = cfgRec; cfgRec.shapeValid = shapeValid('configure', cfgRec.result);
    // load new click for THIS row (reuse path: new audio replacing drained)
    const add = await rpc(pool.node, 'addBuffers', [[makeClick(offset, CLICK_LEN)]]); rpcRecs.addBuffers = add; add.shapeValid = shapeValid('addBuffers', add.result);
    const clearedCap = await rpc(pool.cap, 'clear'); rpcRecs.clearB = clearedCap; clearedCap.shapeValid = shapeValid('clear', clearedCap.result);
    await sleep(CLEAR_TO_START_GAP_MS);
    await sleep(delayMs);
    const tStartCmd = performance.now();
    const started = await rpc(pool.node, 'start', [{ active: true, input: 0, rate: 1 }]); rpcRecs.startB = started; started.shapeValid = shapeValid('start', started.result);
    const startCtxFrame = Math.round(pool.ctx.currentTime * SR);
    rpcRecs.startB.__v627 = { startCommandSentContextFrame: startCtxFrame, startResponseContextFrame: startCtxFrame, requestedStartDelayMs: delayMs, approximate: true };
    const tStartMs = performance.now();
    const declaredLatencyMs = Math.round((pool.latency || 0.06) * 1000);
    const waitMs = Math.ceil((pool.latency || 0.06) * 1000) + 400;
    await sleep(waitMs);
    const statsRec = await rpc(pool.cap, 'stats'); rpcRecs.statsB = statsRec; statsRec.shapeValid = shapeValid('stats', statsRec.result);
    const stats = statsRec.result || {};
    const insRec = await rpc(pool.node, 'getInstrumentMetrics', []); rpcRecs.insB = insRec; insRec.shapeValid = shapeValid('getInstrumentMetrics', insRec.result);
    insRec.result = allowlistInstrumentMetrics(insRec.result || {});
    const ins = insRec.result || {};
    const vsfProf = ins?.vendorStartFrame ?? null;
    const profArgs = vsfProf != null ? [vsfProf + PROFILE_REL_START, vsfProf + PROFILE_REL_END, PROFILE_BIN_SIZE] : [PROFILE_REL_START, PROFILE_REL_END, PROFILE_BIN_SIZE];
    const profRec = await rpc(pool.cap, 'profile', profArgs); rpcRecs.profileB = profRec; profRec.shapeValid = !!profRec.result?.bins;
    const envRec = await rpc(pool.cap, 'envelope', []); rpcRecs.envB = envRec; envRec.shapeValid = !!envRec.result?.env;
    const written = Number(stats.writeIndex || 0);
    const readRec = await rpc(pool.cap, 'readWindow', [0, written]); rpcRecs.readB = readRec; readRec.shapeValid = shapeValid('readWindow', readRec.result);
    readRec.result = readRec.result ?? null;   // (non-lossless path handled by buildRow redaction)
    const chunkId = Math.floor((rep - 1) / CHUNK_ROWS);
    const rowIndex = (rep - 1) % CHUNK_ROWS;
    const row = buildRow({
      kind: 'vendor', offset: PHASE_OFFSET, rep,
      delayIndex, requestedStartDelayMs: delayMs, requestedStartDelaySamples: Math.round(delayMs * SR / 1000), chunkId, globalRowIndex: rep,
      ctx: pool.ctx, cap: pool.cap, vendor: pool.node,
      rpcRecs, stats, ins,
      captured: readRec.result, srcStartFrame: null, deadlineHit: false
    });
    // Condition B attribution + drain proof
    row.raw_observations.conditionId = CONFIG_CONDITION_ID;      // 'B'
    row.raw_observations.poolState = POOL_STATE;                 // 'reused'
    row.raw_observations.r4Lifecycle = {
      condition: 'B', poolState: 'reused',
      rowGapAfterDrainMs: Math.round(drainMarker.elapsedMs),
      drainBeforeRow: drainMarker.drained,
      drainResidualPeak: drainMarker.residualPeak,
      drainResidualEnergy: drainMarker.residualEnergy,
      configureBoundary: rpcRecs.configureB?.result ?? null,
      stopBoundary: drainMarker.stop,
      dropBuffersBoundary: drainMarker.dropBuffers,
    };
    row.raw_observations.completion = {
      conditionId: CONFIG_CONDITION_ID,
      poolState: 'reused',
      rowStartMs: tStartMs,
      rowDoneMs: Math.round(performance.now()),
      rowDurationMs: Math.round(performance.now() - tStartMs),
      deadlineHit: false,
      stopReason: row.verdict?.rowValidity === 'INVALID'
        ? ('invalid:' + String(row.verdict?.invalidReasons?.join('|') || 'unknown'))
        : (drainMarker.drained ? 'completed:b-drained' : 'completed:undrained'),
      origin: 'v6.32.4-audit-r4-conditionB'
    };
    // drain marker: it is REQUIRED for valid rows (verifier rejects missing)
    row.raw_observations.drainMarker = {
      present: true,
      drained: drainMarker.drained,
      cleared: drainMarker.cleared,
      residualPeak: drainMarker.residualPeak,
      residualEnergy: drainMarker.residualEnergy,
      dropBuffers: drainMarker.dropBuffers,
      stopOk: !!drainMarker.stop,
      condition:'B',
      poolState:'reused'
    };
    if (!drainMarker.drained) {
      // drain did not prove zero-residual -> lifecycle mismatch -> INVALID
      row.verdict.rowValidity = 'INVALID';
      row.verdict.invalidReasons = row.verdict.invalidReasons || [];
      row.verdict.invalidReasons.push('drain_marker_undrained');
    }
    // v6.32.3-audit-r2 parity: branchStateId + completion + readPoints-like
    row.raw_observations.conditionId = 'B';
    row.raw_observations.branchStateId = (row.verdict?.rowValidity === 'INVALID')
      ? ('invalid:' + String(row.verdict?.invalidReasons?.join('|') || 'unknown'))
      : 'valid-reuse-path';
    row.raw_observations.captureRequestedLength = written;
    row.raw_observations.captureReturnedLength = Array.isArray(readRec?.result) ? readRec.result.length : (readRec?.result?.length ?? null);
    return row;
  }
  ...
  function finalize() {
    ...
    out.summary = {
      ...
      truncatedRows,
      byOffset,
      // v6.32.3-audit-r3 (r3-instrumentation): completion/state статистика (199 §6.5) — observability-only
      completionStats: {
        poolState: POOL_STATE,
        conditionId: CONFIG_CONDITION_ID,
        rowsWithCompletion: vendorRows.filter(r => !!r.raw_observations?.completion).length,
        rowsWithoutCompletion: vendorRows.length - vendorRows.filter(r => !!r.raw_observations?.completion).length,
        drainStopped: out.stage,
        stopReasonCounts: vendorRows.reduce((a, r) => {
          const s = r.raw_observations?.completion?.stopReason || 'missing';
  ...
  try {
    ...
    // MAND 208 §1.7: runner/source SHA + condition/pool per run (immutable metadata)
    const RUNNER_SHA4 = HARNESS_SHA256;
    out.metadata.runnerSha256 = RUNNER_SHA4;
    out.metadata.sourcePath = SOURCE_PATH;
    out.metadata.conditionId = CONFIG_CONDITION_ID;
    out.metadata.poolState = POOL_STATE;
    ...
    for (let c = 0; c < CHUNKS; c++) {
      ...
      for (const { requestedStartDelayMs, delayIndex, globalRowIndex, chunkId, rowIndex } of chunkRows) {
        if (CONFIG_CONDITION_ID === 'B') {
          // CONDITION B (MAND 208): state-preserving reuse, deterministic drain per row
          await runVendorRowB(requestedStartDelayMs, delayIndex, globalRowIndex);
        } else {
          // CONDITION A (immutable baseline): fresh-per-row (r3 behavior)
          await runVendorRow(requestedStartDelayMs, delayIndex, globalRowIndex);
          // poison fresh pool per row — reuse must NOT leak
        }
      ...
      a.download = 'impulse-v6.32.4-audit-r4-' + CLICK_LEN_LABEL + '-chunk' + c + '-' + TRACK_ID + '-' + Date.now() + '.json';
      a.click();
      log('CHUNK ' + c + ' DONE', { rows: chunkOut.rawObservations.vendorRows.length });
    }
    // Condition B: close reused pool deterministically after run
    if (CONFIG_CONDITION_ID === 'B' && poolB) {
      try { await rpc(poolB.node, 'stop', [{ active: false }]); } catch (e) { out.errors.push('poolB final stop: ' + String(e?.message || e)); }
      try { await poolB.ctx.close(); } catch (e) { out.errors.push('poolB close: ' + String(e?.message || e)); }
    }
    finalize();
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'impulse-v6.32.4-audit-r4-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json';
    a.click();
    log('JSON DOWNLOADED', 'impulse-v6.32.4-audit-r4-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json');
    window.__impulseResultV6324R4 = out;
  } catch (e) {
    out.stage = 'error';
