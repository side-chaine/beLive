;(async () => {
  ...
  const SCRIPT_VERSION = 'v6.32.3-audit';
  const HARNESS_REVISION = 'v6.32.3-audit';               // immutable observability revision (Сол 135)
  const HARNESS_SHA256 = null; // external manifest (П8)
  const SOURCE_PATH = 'src/audio/engine-v3/diagnostics/run-impulse-diagnostics-v6.32.3-audit.console.js';
  ...
  const COMMIT = 'aef5d7ef8119f36eb6ae59239e62c5f92b5883c7';  // Сол 135: baseline-коммит прогона (vendor 55c227ae = тот же, что при v6.32.2; harness immutable)
  ...
  const EXPERIMENT_ID = 'phase-plus-energy-profile-v6.32.3-audit';   // Сол 135: immutable schema version (v6.32.3-audit)
  ...
  const CAPTURE_NAME = 'belive-capture-processor-v6322';
  ...
  window.__impulseResultV6322 = out;

  const log = (...x) => console.log('[v6.32.3-audit]', ...x);
  ...
  function readHealth(ctx, cap, vendor) {
    return {
      ...
      // Сол 135 (C3): vendorProcessCount — число с units ИЛИ literal 'UNAVAILABLE'; silent null запрещён.
      // Vendor counter: 'processCalls' (allowlist, units: process calls); insRec проходит allowlist поэтому processCount недоступен.
      vendorProcessCount: (vendor?.insRec?.processCalls != null) ? { value: vendor.insRec.processCalls, units: 'process calls' } : 'UNAVAILABLE',
  ...
  async function runVendorRow(delayMs, delayIndex, rep) {
    ...
    try {
      ...
      rpcRecs.readPoints = { declaredLatencyMs, readPoints };
      cap.statsRec = stats; cap.statsRec.processCount = stats.processCount;
  ...
  try {
    ...
    for (let c = 0; c < CHUNKS; c++) {
      ...
      a.download = 'impulse-v6.32.3-audit-' + CLICK_LEN_LABEL + '-chunk' + c + '-' + TRACK_ID + '-' + Date.now() + '.json';
      a.click();
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'impulse-v6.32.3-audit-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json';
    a.click();
    log('JSON DOWNLOADED', 'impulse-v6.32.3-audit-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json');
    window.__impulseResultV6322 = out;
  } catch (e) {
    out.stage = 'error';
