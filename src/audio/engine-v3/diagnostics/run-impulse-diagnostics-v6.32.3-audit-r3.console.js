;(async () => {
  ...
  const SCRIPT_VERSION = 'v6.32.3-audit-r3';
  const HARNESS_REVISION = 'v6.32.3-audit-r3';               // immutable observability revision (Сол 137: Fix A readPoints/rpcRecs, Fix B vendorProcessCount)
  const INSTRUMENTATION_R3 = 'completion+state+coords';   // v6.32.3-audit-r3: заверш. трассировка
  const CONFIG_CONDITION_ID = 'A';                        // Block A: текущий read/schedule path (199 §5)
  const POOL_STATE = 'fresh';                             // подтверждено: пула нет, node создаётся заново на каждый row
  const HARNESS_SHA256 = null; // external manifest (П8)
  const SOURCE_PATH = 'src/audio/engine-v3/diagnostics/run-impulse-diagnostics-v6.32.3-audit-r3.console.js';
  ...
  const EXPERIMENT_ID = 'phase-plus-energy-profile-v6.32.3-audit-r3';   // Сол 137: immutable schema version (v6.32.3-audit-r3)
  ...
  window.__impulseResultV6323R3 = out;
  ...
  try {
    ...
    for (let c = 0; c < CHUNKS; c++) {
      ...
      a.download = 'impulse-v6.32.3-audit-r3-' + CLICK_LEN_LABEL + '-chunk' + c + '-' + TRACK_ID + '-' + Date.now() + '.json';
      a.click();
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'impulse-v6.32.3-audit-r3-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json';
    a.click();
    log('JSON DOWNLOADED', 'impulse-v6.32.3-audit-r3-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json');
    window.__impulseResultV6323R3 = out;
  } catch (e) {
    out.stage = 'error';
