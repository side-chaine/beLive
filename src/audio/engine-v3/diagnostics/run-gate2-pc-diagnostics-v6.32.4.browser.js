// ============================================================
// run-gate2-pc-diagnostics-v6.32.4.browser.js
...
//
// MAND 204: vendor resolved via document.baseURI + blob-URL import.
//   - vendor URL is NOT hard-coded to /src or /beLive (404 in the
//     Vite base='/beLive/' setup); it is resolved from document.baseURI.
//   - vendor source is fetched as text and imported through a blob URL
//     (safe: SignalsmithStretch.mjs has no relative import/export/asset
//     dependencies, wasm is inlined), so it executes in the main thread
//     with the worklet mocks already installed.
//   - main-thread class capture via registerProcessor is an ISOLATION MOCK
//     only; it is NOT a real AudioWorkletGlobalScope. Real addModule() proof
//     is a separate browser step (007).
//
...
;(async () => {
  ...
  const SCRIPT_VERSION = 'v6.32.4-pc';
  const HARNESS_REVISION = 'v6.32.4-browser';
  const SOURCE_PATH = 'src/audio/engine-v3/diagnostics/run-gate2-pc-diagnostics-v6.32.4.browser.js';
  const SHA256 = null; // external manifest (007)
  const SEED = 2020807; // deterministic seed (SPEC contract)
  const IS_NODE = typeof process === 'object' &&
    typeof process.versions === 'object' &&
    typeof process.versions.node === 'string';
  const log = (...x) => { if (IS_NODE) console.error('[v6.32.4-pc]', ...x); else console.log('[v6.32.4-pc]', ...x); };
  ...
  function ensureWorkletGlobals() {
    ...
    if (typeof globalThis.registerProcessor === 'undefined') {
      // Isolation mock for main-thread class capture ONLY (not a real
      // AudioWorkletGlobalScope). Keep the first captured class.
      globalThis.registerProcessor = (_name, cls) => { if (!CapturedProcessor) CapturedProcessor = cls; };
    }
  ...
  // ---------- VENDOR IMPORT ----------
  // MAND 204: browser branch resolves the vendor URL from document.baseURI
  // (NOT hard /src or /beLive), fetches it as text, verifies the response,
  // then imports the module through a blob URL. Mocks are installed BEFORE the
  // blob import so the vendor's registerProcessor(...) call is captured into
  // CapturedProcessor (main-thread isolation mock only). Falls back to a
  // root-relative URL resolved against location.origin if the first 404s.
  let Processor = null;
  async function importVendor() {
    if (Processor) return Processor;
    ensureWorkletGlobals();              // mocks BEFORE dynamic import
    if (!IS_NODE) {
      const base = (typeof document !== 'undefined' && document.baseURI) || location.origin + '/';
      const vendorPath = 'src/audio/engine-v3/vendor/SignalsmithStretch.mjs';
      const candidates = [
        new URL(vendorPath, base).href,
        new URL('/src/audio/engine-v3/vendor/SignalsmithStretch.mjs', location.origin).href,
      ];
      const tried = [];
      for (const vendorUrl of candidates) {
        tried.push(vendorUrl);
        try {
          const resp = await fetch(vendorUrl);
          if (!resp.ok) {
            out.errors.push('vendor fetch ' + vendorUrl + ': HTTP ' + resp.status + ' ' + resp.statusText);
            continue;
          }
          const ctype = (resp.headers.get('content-type') || '').toLowerCase();
          if (!/javascript|text|module/.test(ctype)) {
            out.errors.push('vendor fetch ' + vendorUrl + ': unexpected content-type "' + ctype + '"');
            continue;
          }
          const text = await resp.text();
          const blobUrl = URL.createObjectURL(new Blob([text], { type: 'text/javascript' }));
          try {
            await import(/* webpackIgnore */ blobUrl);
            if (CapturedProcessor) { Processor = CapturedProcessor; break; }
          } finally {
            URL.revokeObjectURL(blobUrl);
          }
        } catch (e) {
          out.errors.push('vendor import ' + vendorUrl + ': ' + String(e?.message || e));
        }
      }
      if (!Processor) {
