;(async () => {
  ...
  const SCRIPT_VERSION = 'v6.32.2';
  const HARNESS_REVISION = 'v6.32.2';                     // frozen revision (Sol_2 cond.1)
  ...
  const SOURCE_PATH = 'src/audio/engine-v3/diagnostics/run-impulse-diagnostics-v6.32.2.console.js';
  ...
  const EXPERIMENT_ID = 'phase-plus-energy-profile-v6.32.2';   // v6.32 (MICRO-PACK-095, Сол R67): новый experimentId
  ...
  const log = (...x) => console.log('[v6.32.2]', ...x);
  ...
  try {
    ...
    for (let c = 0; c < CHUNKS; c++) {
      ...
      a.download = 'impulse-v6.32.2-' + CLICK_LEN_LABEL + '-chunk' + c + '-' + TRACK_ID + '-' + Date.now() + '.json';
      a.click();
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'impulse-v6.32.2-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json';
    a.click();
    log('JSON DOWNLOADED', 'impulse-v6.32.2-' + CLICK_LEN_LABEL + '-' + TRACK_ID + '-' + Date.now() + '.json');
    window.__impulseResultV6322 = out;
  } catch (e) {
    out.stage = 'error';
