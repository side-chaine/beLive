...
import { BillyCloud } from './BillyCloud';
...
export function CatalogLayout({ color, onClose }: Props) {
  ...
  const [billyCloudOpen, setBillyCloudOpen] = useState(false);
  const [billyQuestion, setBillyQuestion] = useState('');
  ...
  const handleAskBilly = useCallback((question: string) => {
    ...
    setBillyQuestion(question);
    setBillyCloudOpen(true);
  }, []);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '22% 1fr 28%', gap: 12, flex: 1, minHeight: 0, overflow: 'hidden', padding: '12px 16px 16px' }}>
        ...
        <div style={{ ...colBase, padding: 20 }}>
          ...
          <div style={{
            ...
            <button
              className="bl-catalog-billy-trigger"
              onClick={() => handleAskBilly('С чего начать работу в beLive?')}
            >
              Помочь? 🤔
            </button>
            <div className="bl-catalog-billy-hints">
              {[
