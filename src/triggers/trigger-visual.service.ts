...
// LIFECYCLE OWNER: initTriggerVisualService owns PlaybackVisualScheduler lifecycle (start/stop).
...
/**
 * Trigger Visual Service — Lifecycle owner of PlaybackVisualScheduler
 *
...
export function initTriggerVisualService(): () => void {
  ...
  const reader: PlaybackVisualFrameReader = {
    ...
    read(ctx) {
      ...
      if (transport && ((window as any).__v3Active || (transport.state !== 'idle' && transport.orchestrator.all().length > 0))) {
        // V3 path
        let t = transport.currentTime;
        // Loop wrap: source.loop играет по кругу, но HybridClock.time идёт дальше
        const loop = useLoopStore.getState();
        if (loop.isLooping && loop.loopEndTime != null && loop.loopStartTime != null) {
          const range = loop.loopEndTime - loop.loopStartTime;
          if (range > 0 && t > loop.loopEndTime) {
            t = loop.loopStartTime + ((t - loop.loopStartTime) % range);
          }
        }
        ctx.currentTime = t;
        ctx.isPlaying = transport.state === 'playing';
      } else {
        // V2 fallback
  ...
  if (import.meta.env.DEV) console.log('[TriggerVisualService] initialized');

  return () => {
    initialized = false; // Reset guard for re-init (StrictMode/surface toggle safety)
    stopLoop();
    ...
    if (import.meta.env.DEV) console.log('[TriggerVisualService] disposed');
  };
}
