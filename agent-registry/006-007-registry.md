...
### TASK-015 · V-Mix СТЕРЕО-РАЗВОДКА (vocals L / music center / mic R → MASTER) · [PROPOSAL READY → agent-registry/006-PROPOSAL-TASK-015-VMIX-STEREO.md · ждёт SYNC-OK 007 · применять ПОСЛЕ 463/464]
**Рулинг Ц3:** v-Mix = стерео-разводка, НЕ громкость. Конфиг подтверждён юзером: vocals(стем)→L, music-стемы→center, mic→R, выход MASTER.
**Эталон V2 (frozen-read, TASK-001):** VocalMix.ts — vocals L (merger 0,0 :61), mic R (0,1 :65), music центр (:55-58); ChannelMergerNode, БЕЗ StereoPanner/Delay ⇒ latency 0; на MASTER (ctx.destination :50, AudioEngineV2:548/631).
**Что УЖЕ есть:** mic подключён к router.micInput (TASK-014, каскад живой!); ControlDeck кнопка «VMix OFF/ON» существует; enable/disableVocalMix = фасад no-op (TASK-003).
**Что сделать (спека 006 → пак 007):**
S1: vocalsInput-тап вокального стема (отдельно от programInput-центра).
S2: mic→MAIN путь (сейчас мик только monitorStream!) — при VMix ON мик идёт в R канала мастера.
S3: stereo v-Mix bus: ChannelMergerNode 2ch, без _mainDelay-скева (latency 0).
S4: кнопка VMix ON/OFF в ControlDeck → реальный вкл/выкл разводки (вместо фасада no-op).
S5: stemsEnabled/VMix интеракция (H3.3 глушит music+backing — как дружит с v-Mix?).
**Ограничения:** ❄️ frozen-read эталона ок; single-writer дух C27; latency 0 обязателен. [@PROPOSAL patch] от 006 → SYNC-OK → dispatch 007.
...
- 22.08.2026 (006 · TASK-015 PROPOSAL READY): [@PROPOSAL patch] v-Mix стерео-разводка → `agent-registry/006-PROPOSAL-TASK-015-VMIX-STEREO.md`. КЛЮЧЕВОЕ ОТКРЫТИЕ: вокал УЖЕ идёт отдельным входом vocalHallInput (Orchestrator:51 — только 'vocals') ⇒ S1 без новых тапов вокала; для центра добавлен setVMixCenterTap (пер-стемно, все кроме vocals, аддитивные тапы + addStem-хук). E2: vmix-подграф MonitorRouter (CenterIn→merger ch0+ch1, VocalIn→ch0, MicIn→ch1, merger→_vmixMaster→ctx.destination, БЕЗ delay ⇒ latency 0) + setVMix(on) crossfade defaultBranch↔vmixMaster (иначе задвоение вокала). E3: кнопка VMix → router.setVMix вместо фасада-no-op. S5: stemsEnabled глушит центр автоматически (mute ДО outputNode). Зависимости: применять ПОСЛЕ 463/464 (нужен __belive.monitorRouter и 🎤-тап micInput); mic R слышен только при 🎤 ON. Риски R1-R4 внутри.
