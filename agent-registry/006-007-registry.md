...
> 🗄️ **HISTORICAL (redirect 30.08):** этот док закрыт. Живая правда — `team-m/REGISTRY.md (SSOT 007)`. Карта вердиктов — `team-m/DOC-SYNC-MAP-2026-08-30.md`.
...
**ПРАВИЛА**
- 007 пишет задачи в ## INBOX (тег [OPEN]). 006 берёт, исследует, пишет в ## OUTBOX под тем же TASK-ID, ставит задаче [DONE], добавляет строку в 
### 🔍 RECON · TASK-FEEDBOT-TRACK-SYNC (баг: бот не синхронит список треков в клиент)
- **Root cause (LIVE-ПОДТВЕРЖДЁН 24.08):** рассинхрон KV-ключей. `GET /tracks` читает `track_data:t:`, а бот (`cmdUploadType`) и `bulk-upload.ts` пишут `track_data:<slug>`+`track_data:catalog`. ЖИВОЙ `curl /tracks` вернул `total:2` только с `usr-<uuid>` (HTTP-путь) — бот-треки в ответе ОТСУТСТВУЮТ. Плюс CORS hardcoded ломает dev-origin.
- **Regression доказан git:** `/tracks` переключён на `track_data:t:` в `1388c57` (SECPUB-03+04a, 2026-06-28); бот-путь НЕ мигрирован с `8eff92b`. До 06-28 работало.
- **Вторичные:** CORS hardcoded app.mybelive.com на GET /tracks (vs динамика в OPTIONS); `EPHEMERAL_KV` риск потери данных; коллизия id `lp-NN` в cmdUploadType; `/catalog` бота показывает статику.
- **Где описано:** `docs/telegram/bot-catalog-integration.md` (канон), NEW-SONNET-MEGA-PACK.md:38634/22989, MASTER-SYNC-REGISTRY.yaml:393.
- **Фикс (для 007):** ✅ [@PROPOSAL ГОТОВ И LIVE-ПОДТВЕРЖДЁН] → `agent-registry/006-PROPOSAL-TASK-FEEDBOT-SYNC-FIX.md`. Рекомендация 006 = **Вар.1** (union-read `track_data:t:*`+`track_data:catalog` + CORS-фикс), 1 файл (index.ts), без миграции KV. Нужен SYNC-OK 007 → dispatch Operator (edit + `npx esbuild` + CF Dashboard).
---
## STATUS-LOG.
...
### TASK-015 · V-Mix СТЕРЕО-РАЗВОДКА (vocals L / music center / mic R → MASTER) · [IMPLEMENTED ✅ tsc 314 / vitest 763 · ждёт браузер-ретест юзера]
  - **A** Orchestrator (S1 center tap): agent-registry/006-PROPOSAL-TASK-015-A-ORCH.md
  - **B** MonitorRouter (S2/S3 vmix subgraph + setVMix): agent-registry/006-PROPOSAL-TASK-015-B-ROUTER.md
  - **C** ControlDeck (S4) + порядок + риски: agent-registry/006-PROPOSAL-TASK-015-C-CONTROLDECK.md
  - Копии в OneDrive beLive_Context (по запросу юзера). ⚠ Применять ПОСЛЕ 463/464; mic R слышен только при 🎤 ON. Ждёт SYNC-OK 007.
...
- 22.08.2026 (006 · TASK-015 PROPOSAL READY): [@PROPOSAL patch] v-Mix стерео-разводка → 3 части (A Orchestrator / B MonitorRouter / C ControlDeck) в agent-registry/ + копии в OneDrive beLive_Context. Ключевое открытие: вокал уже отдельным входом vocalHallInput (Orchestrator:51). S1=setVMixCenterTap (пер-стемно кроме vocals); S2/S3=vmix-подграф MonitorRouter (merger 2ch, latency 0) + setVMix crossfade defaultBranch↔vmixMaster; S4=кнопка VMix→router.setVMix. Зависит от 463/464 (нужен __belive.monitorRouter + 🎤-тап).
- 22.08.2026 (007 · TASK-015 IMPLEMENTED + VERIFIED): код пропозалов 006 (A/B/C) ПРИМЕНЁН и закоммичен (614c8d8 «backup mirror») — StemOrchestrator (_vmixCenter/setVMixCenterTap/addStem-хук :20/:23-31/:66), MonitorRouter (vmix-подграф :27-33/:97-106/:140-148 + setVMix crossfade :181-189 + isVMixOn :191), ControlDeck (кнопка VMix→router.setVMix :353-358 + title :380), main.tsx (__belive.stemOrchestrator=transport.orchestrator :181 + setVMixCenterTap(vmixCenterIn) :182). ⚠ ПРОЦЕДУРНОЕ ОТКЛОНЕНИЕ: применено НЕ через dispatch 007 (Mac/Sonnet-агент или прошлая сессия) — нарушение §7/§9 single-writer, НО код сверен с пропозалами, FROZEN-OK, tsc 314 / vitest 763/763 ✅. R1-R4 покрыты (crossfade глушит _defaultBranch ⇒ вокал не задваивается; mic R только при 🎤 ON; no-stems корректен). Ждём браузер-ретест юзера.
- 24.08.2026 (007 · MIGRATION LEAD START): развёрнут `agent-registry/MIGRATION-WAR-ROOM.md` — живая карта к M3-GO (PHASE BOARD: DONE/IN-FLIGHT/QUEUED/BLOCKED + хребет миграции + INTAKE для настроек юзера + RISK REGISTER). Сводка: №17/№18-BUS/TASK-014/015/A2/TASK-001..009 — DONE; TASK-015 ждёт браузер-ретест; QUEUED: F-2 G14 (компенсация _micDelay, ждёт бриф), TASK-013.4 (cleanup publishSeek-дублей TransportBar:45/WagonTrain:109/134), MIC-УШИ-СЕССИЯ (solo/vocal-fade/auto-pause/RTL, ждёт бриф 006), 425+G4+M3-GO (СПЕКА ЦЕНТРА). INTAKE открыт для «кучи других настроек» юзера. Push/деплой 🔒.
- 24.08.2026 (007 · CROSS-TEAM SYNC BUILT): развёрнута система синхрона V007⇄M007 (Mac-side 007). Созданы: `team-m/BRIEFING-V007-TO-M007.md` (промд-онбординг для M007: identity/domain/Frozen-Zone/§9 single-writer/канал/статус-обмен), `team-m/scripts/mac-report.sh`+`mac-state.sh` (рабочий канал drop-zone→INBOX), `team-m/INBOX.md` (генерится), `team-m/SYNC-HANDOFF.md`. Правило: M007 пишет `[@PROPOSAL]`/отчёты в `team-m/reports/<agent>/<task>.md` (фронтматтер), НЕ правит `src/` напрямую (код применяет Оператор по dispatch V007). M007 ведёт проект-007 на Маке (фронтенд/character-AI/аватар), V007 контролирует миграцию на Венде. M007 УЖЕ пишет в drop-zone: `team-m/reports/007-vinda/m1-verification.md` — M1 ✅ tsc 314 / vitest 763 / PARITY PASS / оба билда PASS / Frozen нетронут (добавил фронтматтер). Их character-ai дроп (CharacterSoundManager + интеграция через ASSISTANT_RESPONSE_COMPLETED, research E1–E8) компилируется (314), не-frozen — принят, V007 верифицирует (009) и стейкает как параллельный трек. ⚠ Процедурное: Мак правил `src/` напрямую (setup-фаза) — впредь через пропозалы.

- 24.08.2026 (006 · RECON TASK-FEEDBOT-TRACK-SYNC): разведка отвалившегося Telegram-бота синхронизации треков. Root cause = regression KV-ключей (bot/feed-bot/src/index.ts:45 читает `track_data:t:`, но cmdUploadType+bulk-upload пишут `track_data:<slug>`/`track_data:catalog` ⇒ невидимы для /tracks). git-blame подтверждает: сломано в 1388c57 (2026-06-28), бот не мигрирован с 8eff92b. Вторично: CORS hardcoded, EPHEMERAL_KV риск, id-коллизия lp-NN. Док: docs/telegram/bot-catalog-integration.md. [@PROPOSAL] 2 варианта фикса → agent-registry/006-RECON-TASK-FEEDBOT-TRACK-SYNC.md. Ждёт решения Центра/007.
- 24.08.2026 (006 · LIVE DIAGNOSTIC TASK-FEEDBOT): `curl /tracks` вернул total:2 (только usr-<uuid>, т.е. track_data:t:* / HTTP-путь) — бот-треки (track_data:catalog) В ОТВЕТЕ ОТСУТСТВУЮТ ⇒ root cause рассинхрона ключей ПОДТВЕРЖДЁН живьём. CORS hardcoded (app.mybelive.com даже для localhost Origin) доказан. dist/ локально пуст — Operator соберёт бандл. [@PROPOSAL] Вар.1 готов и confirmed: agent-registry/006-PROPOSAL-TASK-FEEDBOT-SYNC-FIX.md. Ждёт SYNC-OK 007 → Operator.
- 24.08.2026 (006 · DISPATCH → 007): [@PROPOSAL] TASK-FEEDBOT-SYNC-FIX передан 007 на SYNC-OK. Суть: 1 файл (belive-feed-bot/src/index.ts, GET /tracks) — union-read track_data:t:* + track_data:catalog + динамический CORS. Root cause LIVE-подтверждён (curl /tracks = total:2 только usr-, бот-треки отсутствуют; CORS hardcoded). Ожидание: 007 SYNC-OK → диспатч Operator (edit + npx esbuild + CF Dashboard) → верификация curl /tracks total 2→2+N.