...
| нет `fileIds` (справочник, файл не загружен) | `нет файла` | Серый (dim, `opacity:0.7`) |

### Поиск
- `searchQuery >= 2` символов → дропдаун с секциями:
  - **В КАТАЛОГЕ** (зелёный) — совпадения из IDB (формат `Исполнитель — Название`)
  - **В TELEGRAM** (оранжевый) — совпадения из TG (формат `Исполнитель — Название`; fallback: `title` → `artist` → `slug` → `id`)
- Элемент TG-каталога: иконка `☁` + лейбл `Исполнитель — Название`
- **Состояние «нет файла»**: если у трека нет `fileIds.instrumental` / `fileIds.full` (например, хардкод-записи справочника вроде 52 Linkin Park, у которых только метаданные), строка димится (`opacity:0.7`), показывается бейдж `нет файла`, `cursor:help` + tooltip `файл недоступен`, клик не активен — реальной закачки нет, т.к. аудио-файл не загружен в TG/R2. Качать можно только треки с реальным `fileId`.
- ↓ клик по треку С файлом → `downloadTgTrack` → `setSearchQuery('')` → дропдаун закрывается → PORT-призрак качает ZIP
