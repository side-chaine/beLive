...
│    ↓                                                               │
│  track.orchestrator.ts (удалён 01.09, Волна B)                                               │
...
| **W3min** | N-stem loading from IDB stemsData | ✅ FROZEN | `track.orchestrator.ts (удалён 01.09, Волна B)` |
...
| TC-10.9 | Preserve stemsEnabled in initStems + IDB restore | `stem.store.ts`, `track.orchestrator.ts (удалён 01.09, Волна B)`, `audio.bridge.ts` |
...
| TC-10.12 | _stemsBootRestored flag — IDB restore only at boot | `stem.store.ts`, `track.orchestrator.ts (удалён 01.09, Волна B)`, `audio.bridge.ts` |
...
| TC-10.15 | Loading path uses stemsMode, not just stemsEnabled | `track.orchestrator.ts (удалён 01.09, Волна B)` |
