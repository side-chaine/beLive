...
> Якоря: канон 306/772; V2Adapter удаляется ТОЛЬКО после grep→0 (W2 сделала ре-поинт); legacy-путь ИСПРАВЛЕН; live-guard НЕ перемещать (frozen, byte-identical).
...
6. `grep -rln "V2Adapter" src` → 0 (файл удалён); `src/legacy/engine-v3/*` удалён (9 файлов (вкл. 2 test в __tests__)).
