...
> Far: 007_Мак. Hub: 007_Винда применяет post-M3-GO. Источник: FINAL-ROADMAP-draft.md §2 (001, вериф. 002/009). **FIXED per 009 re-verify (26h):** V2Adapter жив (17 importers) → re-point W2 + delete W4 по grep→0; BAC-105 → W1; legacy `src/legacy/engine-v3/*`; BAC-107 → W5; live-guard НЕ moved.
...
| 2 | `MICRO-PACK-WAVE2.md` | delegateSync re-point ×**21** caller + **V2Adapter** ×**17** импортёров re-point; V2Interceptor-wrap/V2Adapter до последнего caller | track.orchestrator режется в В4 |
...
| 4 | `MICRO-PACK-WAVE4.md` | 6 потребителей track.actions; dyn-imports MixerPanel/QuickActions; **legacy ИСПРАВЛЕН: `src/legacy/engine-v3/*` (8 файлов)**; V2Adapter удалить ТОЛЬКО если grep→0; **live-guard НЕ moved** | track.orchestrator/patchV1/AudioEngineV2/bridges read-only |
