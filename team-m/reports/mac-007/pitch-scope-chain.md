...
## КОРРЕКЦИЯ ВЛАДЕЛЬЦА (2026-08-26, post-verdict)
Босс уточнил фактуру бага #6 (008): «80% не точно» — НЕВЕРНО. Наоборот: **~80% детекций БЛИЗКИ К ТОЧНЫМ**, а невнятица/глюки/ложные срабатывания — это **20–30%**. То есть системная ошибка «прыгающий питч / ложные октавы по всей полосе» преувеличена: детектор в основном работает, проблема в интермиттирующих фальш-триггерах, а не в алгоритме целиком.
**СТРАТЕГИЯ БОССА:** не оптимизировать детекцию сейчас. Сначала **поставить всё на V3** (флип миграции), затем ПОСЛЕ миграции, когда скинем кучу legacy-мусора и упорядочим — провести максимальную оптимизацию.
**Следствие для вердикта:** ПОДКЛЮЧЕНИЕ (guard, минимальный путь ~60–80 строк) остаётся **РЕШЕНО** как pre-flip задача; КАЧЕСТВО (автотюн/октавы) **ОТЛОЖЕНО** именно в «большую уборку» post-m3 — где часть глюков 20–30% вероятно уйдёт сама от удаления V2-костылей, а не от смены алгоритма. 002-усиления про autotune (YIN+HPS+октавный консенсус) НЕ актуальны как pre-flip; переносим в post-m3 оптимизацию. То есть: flip-first, optimize-later — ровно то, что выдал Суд.
